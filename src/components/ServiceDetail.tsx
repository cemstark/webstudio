...
export function ServiceDetail({ service }: { service: Service }) {
  ...
  return (
    <>
      ...
      <section className="section"><div className="container split" data-reveal><div><p className="eyebrow">KİMLER İÇİN?</p><h2>Doğru probleme odaklanan kapsam.</h2></div><div><ul className="numberList">{service.audience.map((item) => <li key={item}>{item}</li>)}</ul></div></div></section>
      <section className="section sectionSurface"><div className="container"><SectionHeading eyebrow="KAPSAM" title="İhtiyacı görünür çıktılara çevirin." description={service.summary} /><div className="cardGrid">{service.deliverables.map((item, index) => <article className="serviceCard" key={item} data-stagger><div><p className="micro">0{index + 1}</p><h3>{item}</h3></div></article>)}</div></div></section>
